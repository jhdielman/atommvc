...
class QueryBuilder /*extends PDO*/ {
    ...
    public function __construct($table = null) {
        ...
        $this->from = $table;
        $this->logging = Config::get('database','debug');
    }
    ...
    public function join($table, $one, $operator = null, $two = null, $type = 'inner', $where = false) {
        ...
        // this simple join clauses attached to it. There is not a join callback.
            ...
            if (!$this->hasJoin($table, $one, $operator, $two)) {
    ...
    /**
     * Checks if a join clause already exists.
     * 
     * @param string $table
     * @param string $one
     * @param string $operator
     ...
     * @return boolean
     */
    public function hasJoin($table, $one, $operator = null, $two = null) {
        // Check each join...
        $joinCount = is_array($this->joins) ? count($this->joins) : 1;
        ...
        foreach ($this->joins as $join) {
            ...
            // Is the table the same?
            if ($join->table == $table) {
                
                // Check each clause...
                foreach ($join->clauses as $clause) {
                    
                    // Is the first, operator and second the same?
                    if ($clause["first"] == $one && $clause["operator"] == $operator && $clause["second"] == $two) {
                        return true;
                    }
                }
            }
    ...
    protected function invalidOperatorAndValue($operator, $value) {
        ...
        return ($isOperator && $operator != '=' && $operator != '!=' && is_null($value));
    }
    ...
    public function whereNested(Closure $callback, $boolean = 'and') {
        ...
        // whatever it wants to a query then we will store it for compiling.
        $query = $this->newQuery();
    ...
    protected function run($query, $bindings, Closure $callback) {
        ...
        // lot more helpful to the developer instead of just the database's errors.
            ...
            $this->logQuery($query, $bindings, null);
    ...
    public function logQuery($query, $bindings, $time = null) {
        ...
        if ($bindings) {
            error_log("With parameters: ".implode('; ', array_map(function ($value) {
                if (!isset($value)) {
                    return "<null>";
                }
