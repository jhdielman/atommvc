...
class Security {
    ...
    protected static $antiCsrfMetaName = 'X-CSRFToken';
    ...
    public static function antiCsrfVerified() {
        ...
        if (Request::isSafe() || static::antiCsrfTokensMatch()) {
            $verified = true;
        }
        ...
        Cookie::clear(static::getAntiCsrfCookieName());
    ...
    protected static function antiCsrfTokensMatch() {
        ...
        $csrfTokenName      = static::getAntiCsrfTokenName();
        $csrfToken          = null;
        $csrfTokenExists    = false;
        
        $csrfCookieName     = static::getAntiCsrfCookieName();
        $csrfCookie         = Cookie::value($csrfCookieName);
        $csrfCookieExists   = Cookie::hasValue($csrfCookieName);
        
        $requestHasToken    = Request::hasToken();
        $requestToken       = Request::getToken();
        $postHasToken       = Input::hasPost($csrfTokenName);
        $postToken          = Input::post($csrfTokenName);
        
        if($requestHasToken && $requestToken) {
            $csrfTokenExists = (bool) $requestHasToken;
            $csrfToken = $requestToken;
        } else if($postHasToken && $postToken) {
            $csrfTokenExists = (bool) $postHasToken;
            $csrfToken = $postToken;
        }
        
        $csrfExists = $csrfTokenExists && $csrfCookieExists;
        $csrfMatch = $csrfToken == $csrfCookie;
        
        return (bool) ($csrfExists && $csrfMatch);
    }
    ...
    public static function getAntiCsrfCookieName() {

        return static::$antiCsrfCookieName;
    }

    public static function getAntiCsrfMetaName() {

        return static::$antiCsrfMetaName;
    }
